...
# Re-inventing the wheel with Claude's help ----- 

library(randomForestSRC)
library(tidyverse)

#subsectioning your data 
#function outputs a list of what n you used, n cols actually used, training dataset, testing dataset 
subsect_datasets <- function(data, n) {
  
  outcome_cols <- grep("\\.outcome$", names(data), value = TRUE)
  if (length(outcome_cols) == 0) stop("No columns matching '.outcome' found.")
  
  # ---- identify unique combinations across .outcome columns ----
  combo_key <- do.call(paste, c(data[, outcome_cols, drop = FALSE], sep = "___"))
  n_unique <- length(unique(combo_key))
  
  if (n > n_unique) {
    stop("n (", n, ") exceeds the number of unique .outcome combinations (", n_unique, ").")
  }
  
  # ---- pick one representative row index per unique combo ----
  groups <- split(seq_len(nrow(data)), combo_key)
  representatives <- sapply(groups, function(idx) sample(idx, 1))  # one row per combo
  
  # ---- randomly choose n of those unique representatives for training ----
  train_idx <- sample(representatives, size = n)
...
# ^ can use this function to split dataset into testing and training, w training ranging from 10, 14, 21, 30, 43, 62, 89. Need to make a function for that 

#getting an rf model 
#function outputs 1 or more rf models based on how many desirability traits you have 
build_rf_models <- function(data, ntree = 500, nodesize = 5, seed = NULL) {
  
  # ---- INNER: bin a numeric vector into up to 10 classes (zero, q1..q8, max) ----
  bin_values <- function(x, n_quantiles = 8) {
    nonzero <- x[x > 0 & !is.na(x)]
    if (length(nonzero) == 0) {
      return(factor(ifelse(x == 0, "zero", NA), levels = "zero"))
    }
    max_val <- max(nonzero, na.rm = TRUE)
    probs <- seq(0, 1, length.out = n_quantiles + 1)
    qs <- unique(quantile(nonzero, probs = probs, na.rm = TRUE))  # dedupe edge case
    n_bins <- length(qs) - 1
    
    bins <- character(length(x))
    bins[x == 0] <- "zero"
    bins[x == max_val] <- "max"
    mid <- x > 0 & x < max_val
    ...
    if (n_bins < 1) {
      bins[mid] <- "q1"
      return(factor(bins, levels = c("zero", "q1", "max")))
    }
    
    labels <- paste0("q", seq_len(n_bins))
    bins[mid] <- as.character(cut(x[mid], breaks = qs, include.lowest = TRUE, labels = labels))
    factor(bins, levels = c("zero", labels, "max"))
  }
  
  # ---- identify column groups ----
  outcome_cols <- grep("\\.outcome$", names(data), value = TRUE)
  initial_cols <- grep("\\.initial$", names(data), value = TRUE)      # empty if none present
  desirability_cols <- grep("\\.desirability$", names(data), value = TRUE)
  
  # ---- build predictor set once (shared across all desirability models) ----
  predictor_data <- data[, c(outcome_cols, initial_cols), drop = FALSE]
  
  for (col in outcome_cols) {
    predictor_data[[col]] <- bin_values(data[[col]])   # species abundances -> binned classes
  }
  for (col in initial_cols) {
    if (is.character(predictor_data[[col]])) predictor_data[[col]] <- factor(predictor_data[[col]])
    # e.g. temperature.initial: "hot"/"cold" -> factor, used as-is (categorical, not binned)
  }
  
  mtry_val <- ceiling(sqrt(ncol(predictor_data)))
  
  # ---- train one model per desirability trait ----
  models <- list()
  for (resp in desirability_cols) {
    model_data <- predictor_data
    model_data[[resp]] <- bin_values(data[[resp]])
    form <- as.formula(paste(resp, "~ ."))
    
    if (!is.null(seed)) set.seed(seed)
    models[[resp]] <- rfsrc(form, data = model_data, ntree = ntree, mtry = mtry_val, nodesize = nodesize)
  }
  
  models
}

#get predictions table for classifier rf model 
#function outputs 1 dataset of prediction values 
get_predictions_table <- function(rf_model, training_dataset) {
  predictions_raw <- predict(rf_model, training_dataset) 
  predicts_actual <- as.data.frame(sapply(
    predictions_raw$classOutput, 
    function(x) {x$class}, 
    simplify = FALSE
  ))
  
  return(predicts_actual)
}

get_groundtruth_table <- function (testing_dataset, desirability) {
  #get the testing dataset from subsect_datasets function 
  #get only the desirability column specified from your testing dataset
} 
...
mean_absolute_error_casewise_mean <- function(pred, obs) {
  if(nrow(pred) == nrow(obs) & ncol(pred) == ncol(obs))
  {
    ae_all = sapply(1:nrow(pred), function(i) {
      df_these_desirabilities = data.frame(pred=as.numeric(pred[i,,drop=TRUE]), obs=as.numeric(obs[i,,drop=TRUE]))
      
      # mean absolute error
      ae = mean_absolute_error(df_these_desirabilities$pred, df_these_desirabilities$obs)
      return(ae)
    })
...
# FROM CHATGPT -- ignore for now ---------  
#getting MAE for each desirability trait 
#function outputs a list of results (tibble of interest) and overall mae of the model (useful!) by trait 
evaluate_rf_mae <- function(models, train_data, test_data) {
  
  # ---- INNER: get quantile breakpoints + max value (mirrors build_rf_models' bin_values) ----
  get_breaks <- function(x, n_quantiles = 8) {
    nonzero <- x[x > 0 & !is.na(x)]
    if (length(nonzero) == 0) return(list(qs = numeric(0), max_val = NA_real_))
    max_val <- max(nonzero, na.rm = TRUE)
    probs <- seq(0, 1, length.out = n_quantiles + 1)
    qs <- unique(quantile(nonzero, probs = probs, na.rm = TRUE))
    list(qs = qs, max_val = max_val)
  }
  
  # ---- INNER: bin a vector using PRE-COMPUTED breaks (so test lines up with train's levels) ----
  bin_with_breaks <- function(x, breaks) {
    qs <- breaks$qs
    max_val <- breaks$max_val
    n_bins <- length(qs) - 1
  ...
  # ---- INNER: bin -> numeric midpoint lookup ----
  bin_midpoints <- function(breaks) {
    qs <- breaks$qs
    max_val <- breaks$max_val
    n_bins <- length(qs) - 1
    
    if (n_bins < 1) return(c(zero = 0, q1 = max_val / 2, max = max_val))
    
    q_mids <- map_dbl(seq_len(n_bins), ~ mean(qs[.x:(.x + 1)]))
    names(q_mids) <- paste0("q", seq_len(n_bins))
    c(zero = 0, q_mids, max = max_val)
  }
  
  # ---- INNER: wraps your get_predictions_table, with a fallback if $classOutput isn't populated ----
  # (flagged as untested against univariate models per-trait, per your note)
  get_predictions_safe <- function(rf_model, testing_dataset, trait) {
    predictions_raw <- predict(rf_model, testing_dataset)
    
    has_class_output <- !is.null(predictions_raw$classOutput) && length(predictions_raw$classOutput) > 0
    
    if (has_class_output) {
      # your original logic
      preds_df <- as.data.frame(sapply(
        predictions_raw$classOutput,
        function(x) { x$class },
        simplify = FALSE
      ))
      as.character(preds_df[[1]])  # single-trait model -> take the one column present
    } else {
      # fallback: univariate classification rfsrc objects typically expose $class directly
      if (is.null(predictions_raw$class)) {
        stop("Neither $classOutput nor $class found on predict() output for trait: ", trait)
      }
      as.character(predictions_raw$class)
    }
  }
  
  # ---- rebuild predictor columns for test_data, using TRAIN breaks/levels ----
  outcome_cols <- grep("\\.outcome$", names(train_data), value = TRUE)
  initial_cols <- grep("\\.initial$", names(train_data), value = TRUE)
  
  test_predictors <- test_data[, c(outcome_cols, initial_cols), drop = FALSE]
  for (col in outcome_cols) {
    breaks <- get_breaks(train_data[[col]])
    test_predictors[[col]] <- bin_with_breaks(test_data[[col]], breaks)
  }
  for (col in initial_cols) {
    if (is.character(test_predictors[[col]])) test_predictors[[col]] <- factor(test_predictors[[col]])
  }
  
  # ---- evaluate each trait's model ----
  results_list <- map(names(models), function(trait) {
    
    if (!trait %in% names(test_data)) {
      warning("Skipping ", trait, ": column not found in test_data")
      return(NULL)
    }
    
    model <- models[[trait]]
    breaks <- get_breaks(train_data[[trait]])
    midpoints <- bin_midpoints(breaks)
    
    pred_labels <- get_predictions_safe(model, test_predictors, trait)
    pred_numeric <- unname(midpoints[pred_labels])
    
    per_row <- tibble(
      trait = trait,
      truth = test_data[[trait]],
      pred_label = pred_labels,
      pred_numeric = pred_numeric,
      abs_error = abs(pred_numeric - truth)
    )
    
    list(trait = trait, per_row = per_row, overall_mae = mean(per_row$abs_error, na.rm = TRUE))
  }) %>%
    compact()
  
  names(results_list) <- map_chr(results_list, "trait")
  summary_table <- map_dfr(results_list, ~ tibble(trait = .x$trait, overall_mae = .x$overall_mae))
  
  list(by_trait = results_list, summary = summary_table)
}



#then a function that takes all those outputs into a list and spits out a csv with: replicate, method (rf vs naive), n provided, n used, mae for one desirability with testing, mae for training,  



# JUST A COPY OF COEXISTENCE_LOVE with some var names changed ----- 
# library(MASS)
# library(dplyr)
# library(ggplot2)
# library(terra)
...
# 
# 
# #first get named columns -- see if you're predicting outcomes or abundances 
